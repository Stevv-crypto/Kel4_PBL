        ...
        <form action="{{ route('dataRegister') }}" method="post" class="space-y-8 mt-12">
            @csrf
                ...
                @error('password')
                    <small class="text-red-700">{{ $message }}</small>
                @enderror
                <button href="{{ route('dataRegister') }}" class="block w-full bg-[#70B9EA] text-white py-2 rounded-md text-center hover:bg-blue-600 transition-colors">Create Account</button>
        </form>
